...
class HoneysnapSingleton(Singleton):
    """
    A singleton that stores information about the current honeysnap run.
    This class is used to store arguments and options.
    The goal is to make it easy for various modules to access configuration 
    details without having to be passed an instance of the configuration.
    
    This class is instantiated by calling HoneysnapSingleton.getInstance()
    The first time this is called it must be passed a dictionary as an argument.  All
    subsequent calls will take no arguments.
    """
    
    def __init__(self, options):
        super(HoneysnapSingleton, self).__init__()
        self.options = options
        
    def getOptions(self):
        """
        Returns the instance's options dictionary
        """
        return self.options
        
    def setOption(self, name, value):
        """
        Sets the instance's option dictionary: 
        self.options[name] = value
        Args:
        name: Anything that can be used as a key in a dictionary
        value: Anything that can be a value in a dictionary
        """
        self.options[name] = value
        
    def getOption(self, name):
        """
        Returns the value stored in the options dictionary, or None if name
        is not a valid key
        Args:
        name: Any valid dictionary key
        """
